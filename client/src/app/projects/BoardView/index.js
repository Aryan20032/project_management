...
const TaskColumn = ({ status, tasks, moveTask, setIsModalNewTaskOpen }) => {
  const [{ isOver }, drop] = useDrop(() => ({
    accept: "task",
    drop: (item) => {
      if (item && item.id && status) {
        moveTask({ taskId: item.id, toStatus: status });
      }
    },
    collect: (monitor) => ({
    ...
    <>
      ...
      >
        <div className="mb-3 flex w-full">
          <div
            className={`w-2 !bg-[${statusColor[status]}] -s-lg rounded`}
            style={{ backgroundColor: statusColor[status] }}
          />
          <div className="flex w-full items-center justify-between rounded-e-lg bg-white px-4 py-4 dark:bg-dark-secondary">
  ...
  return (
    ...
    >
      {task.attachments && task.attachments.length > 0 && (
        <Image
          src={`https://pm-s3-images-aryan.s3.us-east-1.amazonaws.com/${task.attachments[0].fileURL}`}
            ...
            <div className="flex space-x-[6px] overflow-hidden">
              {task.assignee && (
                <Image
                  ...
                  src={`https://pm-s3-images-aryan.s3.us-east-1.amazonaws.com/${task.assignee.profilePictureUrl || ""}`}
              ...
              {task.author && (
                <Image
                  ...
                  src={`https://pm-s3-images-aryan.s3.us-east-1.amazonaws.com/${task.author.profilePictureUrl || ""}`}
